...
import { getUserById, CURRENT_USER, BADGES } from "../data/mock";
...
export default function EventDetail() {
  ...
  // Per-member paid amounts (from expenses)
  const paidByMember = {};
  members.forEach((u) => { paidByMember[u.id] = 0; });
  expenses.forEach((exp) => { if (paidByMember[exp.payer] !== undefined) paidByMember[exp.payer] += exp.amount; });
  const maxPaid = Math.max(...Object.values(paidByMember), 1);
  ...
  return (
    <div className="px-5 pt-14 pb-25 overflow-x-clip">
      ...
      {event.status === "settled" ? (
        <>
          {/* Perfectly Bounced banner */}
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 bg-green-900/30 border border-green-800 rounded-full px-4 py-1.5 mb-2">
              <span className="text-sm">✅</span>
              <span className="text-xs font-semibold text-green-400">Perfectly Bounced</span>
            </div>
            <p className="text-xs text-gray-400">
              Marked by {host?.id === CURRENT_USER.id ? "you" : host?.name} on {event.date}
            </p>
          </div>

          {/* Full-width Group Expenses */}
          <div className="text-center mb-8">
            <p className="text-xs text-gray-400 mb-1">Total Group Expenses</p>
            <p className="text-3xl font-bold dark:text-white">${totalCost.toFixed(2)}</p>
          </div>

          {/* Bar chart */}
          <div className="flex items-end justify-around mb-2 px-4">
            {members.map((user) => {
              const paid = paidByMember[user.id] || 0;
              const barPx = maxPaid > 0 ? Math.max(Math.round((paid / maxPaid) * 70), 4) : 4;
              const isTop = paid === maxPaid && paid > 0;
              return (
                <div key={user.id} className="flex flex-col items-center gap-1">
                  <span className="text-[10px] font-semibold text-gray-400">${paid.toFixed(0)}</span>
                  <div
                    className="w-8 rounded-t-md"
                    style={{
                      height: barPx,
                      background: isTop
                        ? "repeating-linear-gradient(0deg, #f87171 0px, #f87171 3px, transparent 3px, transparent 5px)"
                        : "repeating-linear-gradient(0deg, #6b7280 0px, #6b7280 3px, transparent 3px, transparent 5px)",
                    }}
                  />
                </div>
              );
          ...
          <div className="mb-8">
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-3">Individual Expenses</p>
              ...
              {members.map((user) => {
                ...
                return (
                  <div key={user.id} className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-sm dark:text-white">${paid.toFixed(2)}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="h-3 bg-gray-100 dark:bg-card-dark rounded-full overflow-hidden relative">
              <div
                ...
                style={{
                  ...
                  background: settlementPercentage >= 50
                    ...
                    navigate("/send", {
                      state: {
                        ...
                        settleUp: {
                          ...
                          members,
                          currentPercentage: settlementPercentage,
                        },
                      },
                    });
                  }
