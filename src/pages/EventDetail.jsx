...
export default function EventDetail() {
  ...
  return (
    <div className="px-5 pt-14">
      ...
      {/* RSVP swipe button (our proposed feature) */}
      ...
      {rsvpd && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center mb-6">
          ...
          <p className="text-green-600 text-xs mt-0.5">See you there.</p>
        </div>
      )}
      ...
      {(event.status === "splitting" || event.status === "upcoming") && (
        <Link
          to={`/split/${event.id}`}
          className="block w-full text-center bg-black text-white font-semibold py-3.5 rounded-full text-sm"
        >
      ...
      {event.status === "settled" && (
        <div className="bg-gray-50 rounded-xl p-4 text-center">
          <p className="text-sm font-medium text-green-600">All settled!</p>
